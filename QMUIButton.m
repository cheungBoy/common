...
- (void)layoutSubviews {
    ...
    CGSize contentSize = CGSizeMake(CGRectGetWidth(self.bounds)+10 - UIEdgeInsetsGetHorizontalValue(contentEdgeInsets), CGRectGetHeight(self.bounds) - UIEdgeInsetsGetVerticalValue(contentEdgeInsets));
...
- (void)setStatus:(QMUIButtonStatus)status {
    _status = status;
    NSString *imageName = @"condition_btn_icon";
    if (status == QMUIButtonStatusAsc) {
        imageName = @"condition_btn_icon_order_asc";
    }
    if (status == QMUIButtonStatusDesc) {
        imageName = @"condition_btn_icon_order_desc";
    }
